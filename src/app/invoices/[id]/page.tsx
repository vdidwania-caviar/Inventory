...
import { Loader2, AlertTriangle, ArrowLeft, Edit, FileText, Users, CalendarDays, CreditCard, DollarSign } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function fetchInvoiceAndPayments() {
      ...
      try {
        ...
        if (invoiceDocSnap.exists()) {
          ...
          if (currentInvoice.invoiceNumber) {
            const paymentsQuery = query(
              ...
              where('transactionType', '==', 'Sale'), // Payments for invoices are 'Sale' transactions
        ...
        console.error('Error fetching invoice and/or payments:', err);
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 text-sm">
          <div><Users className="inline h-4 w-4 mr-1 text-primary"/><strong>Customer:</strong> {invoice.customerName}</div>
          <div><CalendarDays className="inline h-4 w-4 mr-1 text-primary"/><strong>Invoice Date:</strong> {safeFormatDate(invoice.invoiceDate)}</div>
          <div><CalendarDays className="inline h-4 w-4 mr-1 text-primary"/><strong>Due Date:</strong> {invoice.dueDate ? safeFormatDate(invoice.dueDate) : 'N/A'}</div>
          <div>
            <strong>Status:</strong> <Badge variant={
                invoice.invoiceStatus === 'Paid' ? 'default' : 
                invoice.invoiceStatus === 'Void' || invoice.invoiceStatus === 'Overdue' ? 'destructive' : 
                'secondary'
            }>{invoice.invoiceStatus}</Badge>
          </div>
          <div className="md:col-span-1"><strong>Subtotal:</strong> {formatCurrency(invoice.subtotal)}</div>
          <div className="md:col-span-1"><strong>Tax:</strong> {formatCurrency(invoice.taxAmount)}</div>
          <div className="md:col-span-1 font-semibold"><strong>Total Amount:</strong> {formatCurrency(invoice.totalAmount)}</div>
          <div className="md:col-span-1"><strong>Total Paid:</strong> {formatCurrency(invoice.totalAllocatedPayment)}</div>
          <div className="md:col-span-1 font-semibold"><strong>Balance Due:</strong> {formatCurrency(invoice.totalBalance)}</div>
          ...
          <div className="text-muted-foreground text-xs mt-2"><strong>Created:</strong> {safeFormatDate(invoice.createdAt)}</div>
          <div className="text-muted-foreground text-xs mt-2"><strong>Last Updated:</strong> {safeFormatDate(invoice.updatedAt)}</div>
        </CardContent>
      </Card>
